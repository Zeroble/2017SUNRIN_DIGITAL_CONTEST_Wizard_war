...
app.get('/win', (req, res) => {
  res.sendfile(__dirname + "/gamewin.html")
});
app.get('/lose', (req, res) => {
  res.sendfile(__dirname + "/gamelose.html")
});
class basicObject {
...
class roomObject {
  ...
  update() {
    try {
      for (let i = 0; i < this.userObjects[this.users[0]].skills.length; i++) {
        // console.log(this.userObjects[this.users[1]].x<this.userObjects[this.users[0]].skills[i].x+7,this.userObjects[this.users[1]].x+32>this.userObjects[this.users[0]].skills[i].x+7,this.userObjects[this.users[1]].y<this.userObjects[this.users[0]].skills[i].y+20,this.userObjects[this.users[1]].y+16>this.userObjects[this.users[0]].skills[i].y+20)
        if (this.userObjects[this.users[1]].x < this.userObjects[this.users[0]].skills[i].x + 7 && this.userObjects[this.users[1]].x + 32 > this.userObjects[this.users[0]].skills[i].x + 20 &&
          this.userObjects[this.users[1]].y < this.userObjects[this.users[0]].skills[i].y + 20 && this.userObjects[this.users[1]].y + 40 > this.userObjects[this.users[0]].skills[i].y + 7) {
          this.userObjects[this.users[1]].hp -= 1;
        }
      }
      for (let i = 0; i < this.userObjects[this.users[1]].skills.length; i++) {
        if (this.userObjects[this.users[0]].x < this.userObjects[this.users[1]].skills[i].x + 7 && this.userObjects[this.users[0]].x + 32 > this.userObjects[this.users[1]].skills[i].x + 20 &&
          this.userObjects[this.users[0]].y < this.userObjects[this.users[1]].skills[i].y + 20 && this.userObjects[this.users[0]].y + 40 > this.userObjects[this.users[1]].skills[i].y + 7) {
          this.userObjects[this.users[0]].hp -= 1;
        }
      }
      if(this.userObjects[this.users[0]].hp == 0){
        io.sockets.sockets[this.users[0]].emit('gameResult',"LOSE");
        io.sockets.sockets[this.users[1]].emit('gameResult',"WIN");
      }
      if(this.userObjects[this.users[1]].hp == 1){
        io.sockets.sockets[this.users[0]].emit('gameResult',"WIN");
        io.sockets.sockets[this.users[1]].emit('gameResult',"LOSE");
      }
    }
    catch (e) {
      console.log(e)
...
io.on('connect', (socket) => {
  ...
  socket.on("updateSkill", (data) => {
    try {
      roomObjects[data.roomname].userObjects[socket.id].skills = data.skills
    } catch (e) {
      console.log(e)
    }
  })
